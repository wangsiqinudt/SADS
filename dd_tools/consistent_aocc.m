function [w1,best_para] = consistent_aocc(x, w, fracrej, range, nrbags, varargin)
%CONSISTENT_AOCC
...
%
%
%   [W, PARA] = CONSISTENT_NOCC(X,w,FRACREJ,RANGE,NRBAGS)
%   also returns the optimal combination of hyperparameters PARA.
%
% Default: NRBAGS=5
%
...
%     add_thr = sqrt(fracrej*(1-fracrej)*nrbags/nrx);
    add_thr = sqrt(fracrej*(1-fracrej)/nrx);
else
    add_thr = 0.005;
end
err_thr = sigma_thr*add_thr+fracrej;
...
for t=1:max_times+1
    ...
    % Compute the target error on the leave-out bags
        ...
        w1 = feval(w,xtr,fracrej, para, varargin{:});
        res = dd_error(xte,w1);
        err(i) = res(1);
    end
    fracout = mean(err);
...
%      error('The most simple classifier is still inconsistent!');
    disp('The most simple classifier is still inconsistent!')
    w1 = feval(w, x, fracrej, best_para, varargin{:});
    return;
end

w1 = feval(w, x, fracrej, para, varargin{:});
